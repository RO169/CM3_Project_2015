...
int main()
{
	...
	//front page
	firstImage = al_load_bitmap("startPage.png");
	...
	while (!done)
	{
		...
		if (ev.type == ALLEGRO_EVENT_TIMER)
		{
			...
			else if (state = playing)
			{
				...
				if (points <= 10)						//only intialize aliens untill 20 points
				{
					Enemy[3].startAlien(Enemy,1);
					Enemy[3].updateAliens(Enemy,1);
					
				}
				else if (points <= 20 && points > 10)						//only intialize aliens untill 20 points
				{
					Enemy[3].startAlien(Enemy, 2);
					Enemy[3].updateAliens(Enemy, 2);

				}
				/*if (points <= 30 && points > 20)						//only intialize aliens untill 20 points
				{
					Enemy[3].startAlien(Enemy, 3);
					Enemy[3].updateAliens(Enemy, 3);

				}*/
				
				else						//only intialize aliens untill 20 points
				{
					Enemy[3].startAlien(Enemy, 2);
					Enemy[3].updateAliens(Enemy, 2);
					bss[1].startBoss(bss);
